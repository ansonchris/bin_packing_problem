...
class DPBinner:
    ...
    def get_bin_metrics(self, left, right, k=1):
    ...
    def check_adjacent_and_monotonic(self, edges, k=1, verbose=False):
        ...
        for idx in range(len(edges)-1):
            left = edges[idx]
            right = edges[idx+1]
            metrics = self.get_bin_metrics(left, right, k)
            if metrics is None:
                if verbose:
        ...
        for j in range(2, k+1):
            for i in range(j, micro_bins_count+1):
                for m in range(j-1, i):
                    ...
                    curr_metrics = self.get_bin_metrics(curr_bin_edges[0], curr_bin_edges[1], k=j)
    ...
    def solve_stage1(self, X, Y, n_micro=100):
        ...
        for idx in range(len(best_edges)-1):
            ...
            metrics = self.get_bin_metrics(left, right, k=best_k)
            adjacent_sum = "-"
            if idx < len(best_edges)-2 and best_k > 1:
                next_metrics = self.get_bin_metrics(best_edges[idx+1], best_edges[idx+2], k=best_k)
