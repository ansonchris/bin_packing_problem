...
class DPBinner:
    ...
    def check_adjacent_and_monotonic(self, edges, verbose=False):
        ...
        for idx in range(len(edges)-1):
            ...
            if metrics is None:
                if verbose:
                    print(f"Invalid bin: [{left:.4f}, {right:.4f}] (proportion out of [a%, b%])")
                return False, 0.0, []
    ...
    def find_best_solution_for_k(self, k):
        ...
        for j in range(2, k+1):
            for i in range(j, micro_bins_count+1):
                ...
                for m in range(j-1, i):
                    ...
                    # Check current bin (m -> i) is valid (proportion constraint)
        ...
        if max_iv == -np.inf:
            print(f"No feasible solution for k={k} bins (violates constraints)")
            return False, 0.0, []
...
if __name__ == "__main__":
    ...
    # 故意构造一个可能出现单调性问题的场景（部分区间违约率下降）
    default_prob = np.where(
        X < 90, 0.1,  # 低X：违约率10%
        np.where(X < 110, 0.08,  # 中X：违约率8%（故意低于低X）
                 0.3)  # 高X：违约率30%
    )
    Y = np.random.binomial(1, default_prob)

    # Initialize binner (target_bins=3, increasing monotonicity)
    binner = DPBinner(
