...
class DPBinner:
    def __init__(self, a=5, b=25, c=30, target_bins=5, min_bad_samples=1, monotonicity='increasing', eps=1e-8):
        ...
        self.min_bad_samples = min_bad_samples
    ...
    def get_bin_metrics(self, left, right, k=1, is_last=False):
        ...
        1. Single bin proportion ∈ [a%, b%] (k=1时放宽b为100%)
        2. At least min_bad_samples bad samples (Y=1) in the bin
        Interval logic:
        - is_last=True: [left, right] (closed interval for last bin)
        - is_last=False: [left, right) (left-closed, right-open for other bins)
        """
        # 核心修改：根据is_last控制区间逻辑
        if is_last:
        ...
        # 特殊处理：k=1时单分箱比例上限放宽为100%
        current_a = self.a
        current_b = 1.0 if k == 1 else self.b
        
        # Constraint 1: Single bin proportion ∈ [a%, b%]
        if bin_ratio < current_a - self.eps or bin_ratio > current_b + self.eps:
            return None
    ...
    def check_adjacent_and_monotonic(self, edges, k=1, verbose=False):
        """Check if the binning scheme meets:
        1. Sum of adjacent bin proportions ≤ c% (k=1时无需检查)
        2. Default rate monotonicity (k=1时无需检查)
        ...
        n_bins = len(edges) - 1  # 分箱数 = 边界数 - 1
        for idx in range(n_bins):
            left = edges[idx]
            right = edges[idx+1]
            # 核心修改：判断是否为最后一个分箱（idx == n_bins - 1）
            is_last_bin = (idx == n_bins - 1)
            metrics = self.get_bin_metrics(left, right, k=k, is_last=is_last_bin)
            if metrics is None:
                if verbose:
                    interval_str = f"[{left:.4f}, {right:.4f}]" if is_last_bin else f"[{left:.4f}, {right:.4f})"
                    print(f"Invalid bin: {interval_str} (violates proportion or min bad samples constraint)")
                return False, 0.0, []
            metrics_list.append(metrics)
            total_iv += metrics['iv']
            default_rates.append(metrics['default_rate'])

        # 特殊处理：k=1时无需检查相邻占比和单调性
        if k == 1:
            if verbose:
                print(f"All constraints satisfied (k=1, relaxed rules): Default rate = {[round(dr,4) for dr in default_rates]}")
    ...
    def find_best_solution_for_k(self, k):
        ...
        for i in range(1, micro_bins_count+1):
            ...
            is_valid, iv, _ = self.check_adjacent_and_monotonic(edges, k=1)
            if is_valid:
                dp[i][1] = (iv, None)
        ...
        for j in range(2, k+1):
            for i in range(j, micro_bins_count+1):
                for m in range(j-1, i):
                    ...
                    # Check current bin (m -> i) is valid
                    curr_bin_edges = [self.bin_edges[m], self.bin_edges[i]]
                    # 此处为DP过程中的中间分箱，非最终分箱，is_last=False
                    curr_metrics = self.get_bin_metrics(curr_bin_edges[0], curr_bin_edges[1], k=j, is_last=False)
                    ...
                    is_valid, total_iv, _ = self.check_adjacent_and_monotonic(full_edges, k=j, verbose=False)
                    if is_valid:
        ...
        # 关键修复1：强制包含X的最小值和最大值，确保覆盖所有样本
        final_edges = [self.sorted_X[0]] + final_edges + [self.sorted_X[-1]]
        final_edges = sorted(list(set(final_edges)))  # 去重并保持有序

        # Double validation: Final boundaries must meet all constraints
        final_is_valid, final_total_iv, _ = self.check_adjacent_and_monotonic(final_edges, k=k, verbose=True)
        if not final_is_valid:
            print(f"Unexpected constraint violation for k={k} bins (rejecting solution)")
            return False, 0.0, []

        # 验证边界覆盖所有样本
        if final_edges[0] > self.sorted_X[0] - eps or final_edges[-1] < self.sorted_X[-1] + eps:
            print(f"Warning: Bin boundaries may not cover all samples. Adjusting to include full X range.")
            final_edges[0] = self.sorted_X[0]
            final_edges[-1] = self.sorted_X[-1]
    ...
    def solve_stage1(self, X, Y, n_micro=100):
        ...
        bin_metrics = []
        n_bins = len(best_edges) - 1  # 分箱数 = 边界数 - 1
        for idx in range(n_bins):
            left = best_edges[idx]
            ...
            metrics = self.get_bin_metrics(left, right, k=best_k, is_last=is_last_bin)
            adjacent_sum = "-"
            if idx < n_bins - 1 and best_k > 1:
                next_metrics = self.get_bin_metrics(best_edges[idx+1], best_edges[idx+2], k=best_k, is_last=(idx+1 == n_bins - 1))
        ...
        total_ratio = 100.0  # 总占比100%
        total_non_default = self.total_non_default
        total_default = self.total_default
        total_default_rate = (total_default / total_sample_count * 100) if total_sample_count > 0 else 0.0
        total_iv = sum([m['IV Value'] for m in bin_metrics])

        # 验证分箱样本数总和与总样本数一致（区间逻辑修正后应无偏差）
        bin_sample_sum = sum([m['Sample Count'] for m in bin_metrics])
        if abs(bin_sample_sum - total_sample_count) > eps:
            print(f"Warning: Sum of bin sample counts ({bin_sample_sum}) does not match total samples ({total_sample_count}). Adjusting to ensure coverage.")
            # 若有样本遗漏，强制最后一个分箱包含剩余样本（极端情况处理）
            last_bin = bin_metrics[-1]
            last_bin['Sample Count'] = total_sample_count - sum([m['Sample Count'] for m in bin_metrics[:-1]])
            last_bin['Non-Default Count'] = total_non_default - sum([m['Non-Default Count'] for m in bin_metrics[:-1]])
            last_bin['Default Count'] = total_default - sum([m['Default Count'] for m in bin_metrics[:-1]])
            last_bin['Ratio (%)'] = round(last_bin['Sample Count'] / total_sample_count * 100, 2)
            last_bin['Default Rate (%)'] = round(last_bin['Default Count'] / last_bin['Sample Count'] * 100, 2)
            # 重新计算最后一个分箱的IV
            last_iv = calculate_iv(
                last_bin['Non-Default Count'], last_bin['Default Count'],
                self.total_non_default, self.total_default
            )
            last_bin['IV Value'] = round(last_iv, 6)
            total_iv = sum([m['IV Value'] for m in bin_metrics])

        # 追加 Total 行
        total_row = {
            'Bin Interval': 'Total',
            'Sample Count': total_sample_count,
            'Ratio (%)': total_ratio,
            'Non-Default Count': total_non_default,
            'Default Count': total_default,
            'Default Rate (%)': round(total_default_rate, 2),
            'WOE Value': '-',
            'IV Value': round(total_iv, 6),
            'Adjacent Ratio Sum (%)': '-',
            'Raw Ratio': 1.0,
            'Raw Default Rate': total_default / total_sample_count if total_sample_count > 0 else 0.0
        }
        bin_metrics.append(total_row)
        ...
        print(f"Validation: Total Samples = {total_sample_count}, Total Defaults = {total_default} (matches Y sum: {self.total_default})")
    ...
    def validate_bin_result(self):
        ...
        metrics_df = self.bin_result['Bin Detailed Metrics'].iloc[:-1]  # 排除Total行
        a = self.a * 100
        b = self.b * 100
        c = self.c * 100
        min_bad = self.min_bad_samples
        monotonicity = self.monotonicity
        best_k = self.bin_result['Optimal Bin Count']
        ...
        # Validation 2: Each bin proportion ≤ b% (k=1时放宽为100%)
        current_b = 100.0 if best_k == 1 else b
        high_ratio_bins = metrics_df[metrics_df['Ratio (%)'] > current_b + self.eps]['Bin Interval'].tolist()
        validate_result['Constraint Item'].append(f'Maximum Single Bin Proportion ≤ {current_b}%')
        ...
        validate_result['Remarks'].append(f'b = {b}% (k=1时放宽为100%)')

        # Validation 3: Sum of adjacent bin proportions ≤ c% (k=1时无需检查)
        ...
        validate_result['Validation Result'].append('Pass' if (best_k == 1 or not exceed_adjacent_bins) else 'Fail')
        validate_result['Non-Compliant Bins'].append('; '.join(exceed_adjacent_bins) if exceed_adjacent_bins else '-')
        validate_result['Remarks'].append(f'c = {c}% (k=1时无需检查)')
        ...
        # Validation 5: Default rate monotonicity (k=1时无需检查)
        ...
        validate_result['Validation Result'].append('Pass' if (best_k == 1 or is_monotonic) else 'Fail')
        validate_result['Non-Compliant Bins'].append('; '.join(non_monotonic_pos) if non_monotonic_pos else '-')
        validate_result['Remarks'].append(f'Requirement: {monotonicity} (k=1时无需检查)')

        # Validation 6: Total sample count and default count match
        total_sample_count = self.bin_result['Bin Detailed Metrics'].iloc[-1]['Sample Count']
        total_default_count = self.bin_result['Bin Detailed Metrics'].iloc[-1]['Default Count']
        sample_match = abs(total_sample_count - self.total_samples) < eps
        default_match = abs(total_default_count - self.total_default) < eps
        validate_result['Constraint Item'].append('Total Sample & Default Count Match')
        validate_result['Validation Result'].append('Pass' if sample_match and default_match else 'Fail')
        validate_result['Non-Compliant Bins'].append('-' if sample_match and default_match else f"Sample count mismatch: {total_sample_count} vs {self.total_samples}; Default count mismatch: {total_default_count} vs {self.total_default}")
        validate_result['Remarks'].append('Sample count must match X length; Default count must match Y sum')
    ...
    def export_results(self):
        ...
        Returns:
            bin_metrics_df: Bin detailed metrics DataFrame (with Total row)
...
if __name__ == "__main__":
    # Generate test data (60,000 samples) with strictly increasing default rate
    np.random.seed(42)
    n_samples = 60000
    X = np.random.normal(100, 20, n_samples)
    # 修正：构造严格递增的违约率（匹配monotonicity='increasing'）
    default_prob = np.where(
        X < 80, 0.05,  # 低X：5%违约率
        np.where(X < 100, 0.15,  # 中X：15%违约率
                 np.where(X < 120, 0.25, 0.35))  # 高X：25%/35%违约率（严格递增）
    )
    Y = np.random.binomial(1, default_prob)

    # Initialize binner (优化参数：c调整为40%避免相邻占比超限，b保持25%但k=1时自动放宽)
    binner = DPBinner(
        a=5,              # 单分箱最小占比5%
        b=25,             # 单分箱最大占比25%（k=1时自动放宽为100%）
        c=40,             # 相邻分箱占比和最大40%
        target_bins=5,    # 目标分箱数5
        min_bad_samples=5, # 每个分箱至少5个坏样本
        monotonicity='increasing'  # 匹配修正后的测试数据单调性
    )
    ...
    print("="*120)
    print("\nBin Detailed Metrics DataFrame (with Total Row):")
