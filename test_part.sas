
proc import datafile = "C://Users//wangyan//Desktop//df_all.csv"
	out = all_data
	dbms = csv
	replace;
	getnames = yes;
run;

data all_data;
	set all_data;
	pd = pd * 100000;
run;


/****************************************************************************************/
proc iml;
/* 1. Global Constraints [cite: 10-11] */
a = 0.05; b = 0.25; C = 0.50; eps = 1e-7;
n_train = 10; n_test = 6;

/* Load data from table into matrices */
use all_data; 
    read all var {pd target ds_idx is_train} into raw_data;
close all_data;

n_row = nrow(raw_data);
COLNAME ={"pd","target","ds_idx","is_train"};
row_labs = char(1:n_row);
MATTRIB raw_data ROWNAME = row_labs COLNAME = COLNAME;


/* --- CORE METRIC CALCULATOR  --- */
start get_bin_metrics(low, high, is_last, data, a, b, C, eps, n_tr, n_ts);
    n_ds = n_tr + n_ts;
    counts = j(1, n_ds, 0); 
	bads = j(1, n_ds, 0); 
	props = j(1, n_ds, 0);
    
    do k = 1 to n_ds;
        idx_all = loc(data[,3] = k);
		ds_all = data[idx_all, ];

        *if is_last then idx = loc(data[,3] = k & data[,1] >= low & data[,1] <= high);
        *else            idx = loc(data[,3] = k & data[,1] >= low & data[,1] < high);

		if is_last then idx = loc(ds_all[,1] >= low & ds_all[,1] <= high);
        else            idx = loc(ds_all[,1] >= low & ds_all[,1] < high);
        
        if ncol(idx) > 0 then do;
            counts[k] = ncol(idx);
            bads[k] = ds_all[idx, 2][+];
            props[k] = counts[k] / ncol(idx_all);
			* msg = cats("DS:",k,"--low:", low,"--high", high ,"--Bads:", bads[k],"--Counts:", counts[k], "--Prop:", props[k], "--ds_size:", ncol(idx_all),"--other:", nrow(ds_all));
			* print msg;
        end;
    end;

    /* Condition 1: Individual dataset proportion check [cite: 31-33] */
	*print "start from here";
	if any(props < a - eps) | any(props > b + eps) then return(j(1, 10, .));

    total_tr_pop = nrow(loc(data[,4]=1));
    total_ts_pop = nrow(loc(data[,4]=0));
    tr_cnt = counts[1:n_tr][+]; 
	tr_bad = bads[1:n_tr][+];
    ts_cnt = counts[n_tr+1:n_ds][+]; 
	ts_bad = bads[n_tr+1:n_ds][+];
    
    *tr_br = (tr_cnt > 0) ? tr_bad/tr_cnt : 0;
    *ts_br = (ts_cnt > 0) ? ts_bad/ts_cnt : 0;

	if tr_cnt > 0 then
		tr_br = tr_bad/tr_cnt;
	else tr_br = 0;

	if ts_cnt > 0 then
		ts_br = ts_bad/ts_cnt;
	else ts_br = 0;


    /* Avg Test Bad Rate [cite: 43] */
    ts_br_list = j(1, n_ts, 0);
    do k = 1 to n_ts;
        if counts[n_tr+k] > 0 then ts_br_list[k] = bads[n_tr+k]/counts[n_tr+k];
    end;
    avg_ts_br = ts_br_list[:];

    /* Return vector of metrics for DP storage */
	msg = cats("tr_br:", tr_br, "-ts_br:", ts_br, "-avg_ts_br:",avg_ts_br, "-tr_cnt/total_tr_pop:", tr_cnt/total_tr_pop, "-ts_cnt/total_ts_pop:", ts_cnt/total_ts_pop, "-props:", props);
	*print msg;
    return (tr_br || ts_br || avg_ts_br || (tr_cnt/total_tr_pop) || (ts_cnt/total_ts_pop) || props);
finish;

/* --- STAGE 1: DYNAMIC PROGRAMMING  --- */
start solve_stage1(data, a, b, C, eps, n_tr, n_ts);
    pd_col = data[,1];
    call qntl(edges, pd_col, do(0, 1, 1/100)); /* n_micro=100 [cite: 55, 59] */
    edges = unique(edges);
    n_e = ncol(edges) - 1;
    
    /* chains stores: [len, pred_idx, tr_br, ts_br, avg_ts_br, props(1:16)] */
    chains = j(n_e, n_e, .); 
    lengths = j(n_e, n_e, 0);
    
    do j = 1 to n_e;
        do i = 0 to j-1;
            curr = get_bin_metrics(edges[i+1], edges[j+1], (j=n_e), data, a, b, C, eps, n_tr, n_ts);
            if any(curr = .) then continue;
            
            if i = 0 then lengths[i+1, j] = 1;
            else do;
                best_val = -1; best_p = -1;
                do p_start = 1 to i;
                    if lengths[p_start, i] > 0 then do;
                        prev = get_bin_metrics(edges[p_start], edges[i+1], 0, data, a, b, C, eps, n_tr, n_ts);
                        /* Monotonicity & Adj Sum checks [cite: 74-79] */
                        if curr[1] <= prev[1]-eps | curr[2] <= prev[2]-eps | curr[3] <= prev[3]-eps then continue;
                        if any(prev[6:21] + curr[6:21] > C + eps) then continue;
                        
                        if lengths[p_start, i] > best_val then do;
                            best_val = lengths[p_start, i];
                            best_p = p_start;
                        end;
                    end;
                end;
                if best_p > 0 then lengths[i+1, j] = best_val + 1;
            end;
        end;
    end;
    
    /* Backtrack Path [cite: 85-89] */
    max_len = lengths[, n_e][<>];
    if max_len = 0 then return(.);
    curr_j = n_e;
    curr_i = loc(lengths[, n_e] = max_len)[1];
    path = edges[curr_j+1];
    do while (curr_i > 1);
        path = edges[curr_i] || path;
        next_j = curr_i - 1;
        /* Find predecessor that led to this max length */
        curr_i = loc(lengths[, next_j] = (lengths[curr_i, curr_j]-1))[1];
        curr_j = next_j;
    end;
    return (edges[1] || path);
finish;


/* --- STAGE 2: RECURSIVE REPAIR  --- */
start repair_recursive(cutoffs, data, a, b, C, eps, n_tr, n_ts) ;
    n_bins = ncol(cutoffs) - 1;
    if n_bins < 1 then return(.);
    
    stats = j(n_bins, 5 + (n_tr + n_ts), 0);
    do i = 1 to n_bins;
        m = get_bin_metrics(cutoffs[i], cutoffs[i+1], (i=n_bins), data, a, b, C, eps, n_tr, n_ts);
        if any(m = .) then return(.);
        stats[i,] = m;
    end;

    violation_idx = 0;
    do i = 2 to n_bins;
        prev = stats[i-1,]; curr = stats[i,];
        mono_fail = (curr[1] <= prev[1]-eps) | (curr[2] <= prev[2]-eps) | (curr[3] <= prev[3]-eps);
        adj_fail = any(prev[6:21] + curr[6:21] > C + eps);
        if mono_fail | adj_fail then do; violation_idx = i; leave; end;
    end;

    if violation_idx = 0 then return(cutoffs); /* Base Case [cite: 120] */

    /* Strategies: L, R, LL [cite: 123-140] */
    /* Strategy L: Remove cutoffs[violation_idx] */
    idx_l = loc(1:ncol(cutoffs) ^= violation_idx);
    res_l = repair_recursive(cutoffs[idx_l], data, a, b, C, eps, n_tr, n_ts);
    
    /* Strategy R: Remove cutoffs[violation_idx + 1] */
    res_r = .;
    if violation_idx < n_bins then do;
        idx_r = loc(1:ncol(cutoffs) ^= (violation_idx + 1));
        res_r = repair_recursive(cutoffs[idx_r], data, a, b, C, eps, n_tr, n_ts);
    end;

    /* Strategy LL: Remove cutoffs[violation_idx - 1] */
    res_ll = .;
    if violation_idx >= 3 then do;
        idx_ll = loc(1:ncol(cutoffs) ^= (violation_idx - 1));
        res_ll = repair_recursive(cutoffs[idx_ll], data, a, b, C, eps, n_tr, n_ts);
    end;

    /* Return longest path [cite: 144] */
    max_len = 0; final_res = .;
    if ncol(res_l) > max_len then do; max_len = ncol(res_l); final_res = res_l; end;
    if ncol(res_r) > max_len then do; max_len = ncol(res_r); final_res = res_r; end;
    if ncol(res_ll) > max_len then do; max_len = ncol(res_ll); final_res = res_ll; end;
    
    return(final_res);
finish;

/* --- EXECUTION --- */
stage1_cuts = solve_stage1(raw_data, a, b, C, eps, n_train, n_test);
print "stage1_cuts:", stage1_cuts;
final_cuts = repair_recursive(stage1_cuts, raw_data, a, b, C, eps, n_train, n_test);

/* Export for PROC REPORT [cite: 172-207] */
*create final_bin_edges from final_cuts; 
*append from final_cuts; 
*close final_bin_edges;

create final_bin_edges from stage1_cuts; 
close final_bin_edges;

quit;

